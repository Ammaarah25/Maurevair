 ...
 * 
 * //retrieve username from jwt token public String getUsernameFromToken(String
 * token) { return getClaimFromToken(token, Claims::getSubject); }
 * 
 * //retrieve expiration date from jwt token public Date
 * getExpirationDateFromToken(String token) { return getClaimFromToken(token,
 ...
 * claimsResolver.apply(claims); } //for retrieveing any information from token
 * we will need the secret key private Claims getAllClaimsFromToken(String
 * token) { return
