 ...
 * 
 * import com.maurevair.booking.service.JwtUserDetailsService;
 * 
 ...
 * 
 * String username = null; String jwtToken = null; // JWT Token is in the form
 * "Bearer token". Remove Bearer word and get // only the Token if
 ...
 * authentication if (jwtTokenUtil.validateToken(jwtToken, userDetails)) {
 * 
 * UsernamePasswordAuthenticationToken usernamePasswordAuthenticationToken = new
 * UsernamePasswordAuthenticationToken( userDetails, null,
 * userDetails.getAuthorities()); usernamePasswordAuthenticationToken
 * .setDetails(new WebAuthenticationDetailsSource().buildDetails(request)); //
 * After setting the Authentication in the context, we specify // that the
